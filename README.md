  ...
  **该项目是一个支持机器学习和深度学习的多因素时间序列预测系统，旨在能够根据历史数据和可能影响时间序列的外部因素预测时间序列的未来值。**
  ...
  1. 打开 **project_parameters.py** 文件中的 **ProjectParameters** 类，自定义保存相关参数。**self.file_name** 变量存放的是输入文件的文件名（支持 csv、xls、xlsx 格式），需要将**文件放置在根目录下的 Datasets 目录下**（如果不存在应当手动创建）。
  2. 打开 **data_parameters.py** 文件中的 **DataParameters** 类，自定义数据处理相关参数。用户要将 **self.target, self.time_unknown_variables 和 self.time_known_variables** 三个参数修改为自己文件的形式，否则会找不到特征和目标。
  3. 打开 **predictor_parameters.py** 文件中的 **PredictorParameters** 类，自定义预测模型相关参数。
  4. 打开 **ensemble_parameters.py** 文件中的 **EnsembleParameters** 类，自定义组合模型相关参数。
  5. 【可选】打开 **DLCriterion.py** 文件，自定义损失类和监控器类，并在 **main.py** 文件中深度学习模型参数 **criterion** 和 **monitor** 修改为自定义的类。
  6. 打开 **main.py** 文件，选择运行预测器（支持运行单个预测器或多个预测器）还是集成器（支持运行单个集成器或多个集成器）（支持单独执行预测器或者集成器，也支持同时运行预测器和集成器）。
  ...
  - **project_parameters.py** 文件中的 **ProjectParameters** 类存放读入数据文件名、运行结果的保存形式和地址。
  ...
  - **RUNER.py** 为预测器和集成器的封装程序。主要内容包括：将预测器和集成器的参数写入本地、将原始数据封装为机器学习和深度学习格式的数据集、运行预测器、运行集成器、保存日志、将所有暂存内容全部写入本地文件。（程序运行过程中如果遇到任何继承于 Exception 的错误，都将会保存错误至 **documents\\Logs.log** 下，然后将所有暂存的结果全部保存到本地，防止数据丢失）。
  ...
  5. 在 **project_parameters.py** 文件中的 **ProjectParameters** 类中调整读取设置、保存设置和路径设置。
  6. 在 **data_parameters.py** 文件中的 **DataParameters** 类中调整数据处理（数据封装）相关的参数。
  7. 在 **predictor_parameters.py** 文件中的 **PredictorParameters** 类中调整模型相关的参数。**如果有自定义模型，一定要在这里添加模型参数，类内变量名应当与类名保持一致。**
  8. 在 **ensemble_parameters.py** 文件中的 **EnsembleParameters** 类中调整模型相关的参数。**如果有自定义模型，一定要在这里添加模型参数，类内变量名应当与类名保持一致。**
  9. 打开 **main.py** 文件，根据您的需求选择不同的运行模式（文件内提供了 7 种不同的运行形式），所有运行形式可以分为以下三类：
     1. 只运行预测器：选择您所需要运行的单个预测器或多个预测器，然后运行 **main.py** 文件。
     2. 只运行集成器：选择您所需要运行的单个集成器或多个集成器，然后运行 **main.py** 文件。
     3. 同时运行预测器和集成器（不推荐）：选择您所需要运行的预测器和集成器，然后运行 **main.py** 文件。
  10. 打开保存路径查看预测结果或集成结果。
