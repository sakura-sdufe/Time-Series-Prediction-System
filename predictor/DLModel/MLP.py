...
class MLPModel(ModelBase):
    def __init__(self, input_size, output_size, time_step, *, hidden_sizes:Sequence[int]=None, activation='relu',
                 dropout=0.1):
        """
        初始化多层感知机模型。输入层节点个数为：input_size*num_predictors，输出层节点个数为：output_size。
        ...
        if hidden_sizes is None:
            hidden_sizes = []
        assert hidden_sizes[0] > 0, "hidden_sizes 的第一个元素必须大于 0，因为输入层的维度是 input_size*num_predictors。"
    ...
    def forward(self, X):
        ...
        """
        X = X.permute(1, 0, 2)  # 调整维度为 [batch_size, num_predictors, input_size]
        X = X.reshape(X.shape[0], -1)  # 调整维度为 [batch_size, input_size*num_predictors]
