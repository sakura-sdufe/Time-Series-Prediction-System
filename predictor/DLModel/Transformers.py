...
class PositionalEncoding(nn.Module):
    def __init__(self, model_dim, max_length=1000, dtype=torch.float32):
        ...
        初始化位置编码，为位置编码创建正弦模式。
        Args:
            model_dim: TransformerWithLinear 模型维度。
...
class AttentionLinearLayer(nn.Module):
    ...
    def forward(self, src):
        ...
        src = self.norm1(src)
        # Feedforward
        linear_output = self.dropout(self.activation(self.linear1(src)))
        linear_output = self.dropout(self.linear2(linear_output))
...
class TransformerWithLinear(ModelBase):
    def __init__(self, input_size, output_size, *,
        ...
        """
        初始化 TransformerWithLinear 预测模型。编码器为 TransformerEncoder，解码器为全连接层。
        ...
        """
        super(TransformerWithLinear, self).__init__()
        ...
        # 解码器
        decoder_layers = []
        previous_size = encoder_model_dim
        if decoder_hidden_sizes:
            for hidden_size in decoder_hidden_sizes:
                decoder_layers.append(nn.Linear(previous_size, hidden_size))
...
class TransformerWithAttention(ModelBase):
    def __init__(self, input_size, output_size, *,
                 encoder_model_dim=128, encoder_head_num=8, encoder_feedforward_dim=2048, encoder_layer_num=2,
        ...
        """
        初始化 TransformerWithAttention 预测模型。编码器为 TransformerEncoder，解码器为 MultiHeadAttention。
        :param input_size: 输入特征维度。
        :param output_size: 输出特征维度。
        :param encoder_model_dim: 编码器 TransformerEncoderLayer 模型维度，默认值为 128。
        :param encoder_head_num: 编码器 TransformerEncoderLayer 多头注意力机制的头数，默认值为 8。
