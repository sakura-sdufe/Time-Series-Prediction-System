...
from .predictor_ML import split_dataset
...
class SeqDataset(Dataset):
    def __init__(self, feature, target):
        ...
        :param feature: 3D Tensor, 维度为：(sample_number, num_predictors, number)。
        :param target: 1D Tensor, 维度为：(sample_number,)。
        """
        self.feature = feature
        self.target = target
        self.sample_number = feature.shape[0]
        self.time_step = feature.shape[1]
        self.input_size = feature.shape[2]

    def __getitem__(self, item):
        """
        根据索引返回数据集中的一个样本。
        :param item: 表示需要获取的样本索引，int 类型。
        :return: 返回一个 torch.Tensor 类型的样本，特征的尺寸为 (num_predictors, number)，目标的尺寸为 (1,)。
        """
...
class SeqLoader:
    def __init__(self, time_feature, time_target, parameters_data, normalization=None):
        ...
        # 生成 DataLoader 类型数据（未标准化）
        self.train_dataloader_origin, self.train_eval_dataloader_origin, \
            self.valid_dataloader_origin, self.test_dataloader_origin = None, None, None, None
        self._generate_loader_origin()
        # 生成 DataLoader 类型数据（标准化）
        if normalization:
            self.normalization_feature, self.normalization_target = None, None  # 用于标准化的类
            self.train_dataloader_norm, self.train_eval_dataloader_norm, \
                self.valid_dataloader_norm, self.test_dataloader_norm = None, None, None, None
    ...
    def _generate_loader_origin(self):
        ...
        # 将时序的特征和目标转为样本的特征和目标，特征维度：(sample_number, num_predictors, number)，目标维度：(sample_number,)
        ...
        self.train_dataloader_origin = DataLoader(self.train_dataset_origin,
                                                  batch_size=self.parameters_data["predictor_batch_size"],
